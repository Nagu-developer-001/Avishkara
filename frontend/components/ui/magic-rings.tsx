...
type MagicRingsSettings = Required<Omit<MagicRingsProps, "className" | "style">>;
...
export default function MagicRings({
  ...
  className,
  style,
}: Readonly<MagicRingsProps>) {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const propsRef = useRef<MagicRingsSettings>({
    color,
    colorTwo,
    speed,
    ringCount,
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      uniforms.uTime.value = time * 0.001 * settings.speed;
      uniforms.uAttenuation.value = settings.attenuation;
      uniforms.uColor.value.set(settings.color);
      uniforms.uColorTwo.value.set(settings.colorTwo);
